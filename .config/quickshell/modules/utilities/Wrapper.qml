...
Item {
    ...
    // R-custom: zen-mode lockdown (plan zen-mode task-6) — utilities cannot
    // render while zen is on (covers the attached-to-sidebar state too).
    readonly property bool shouldBeActive: !ZenMode.enabled && (screenState.sidebar || (screenState.utilities && Config.utilities.enabled && !(screenState.session && Config.session.enabled)))
