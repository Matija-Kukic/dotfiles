...
My themes live in `.config/colorschemes/`: `monet`, `catppuccin-mocha`, `everforest`, and `gruvbox`. I switch them with `switch-theme <name>` or the launcher's `> scheme` action, and kitty, alacritty, tmux, nvim+lualine, hyprland, quickshell+wallpaper, opencode, zen-browser, GTK, and oh-my-posh all follow.
